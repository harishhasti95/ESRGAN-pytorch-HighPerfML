...
parser.add_argument('--masks_pth_path', default='parameters/masks.pth')
...
# Use pruner to save the new weights
with torch.no_grad():
    net = ESRGAN(3, 3, scale_factor=4)
    net.load_state_dict(net_interp)
    net = net.to(device).eval()
    ...
    torch.save(load_dict, args.masks_pth_path)

# Load the pruned model to test for PSNR Ratio
with torch.no_grad():
    net = ESRGAN(3, 3, scale_factor=4)
    net.load_state_dict(torch.load(args.masks_pth_path))
    net = net.to(device).eval()
